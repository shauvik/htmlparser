...
public class TagTests extends ParserTestCase {
  ...
  /* 
   * This has to be volatile as otherwise the do-while loop in test 
   * testThreadSafety becomes infinite because if a while loop has
   * a variable inside its condition that is not being changed inside
   * loop then the JVM optimizes this loop by copying the value of the
   * variable instead of checking it every time.
   */
  private volatile int testProgress;
  /*
   * This lock is also used to make the do-while loop inside the test
   * testThreadSafety as the statement testProgress += mId inside the 
   * finally block is not thread safe as the result many threads who
   * try to increase the value to shared variable that is testProgress
   * do it concurrently leading concurrency issues.
   * NOTE: However assigning to an int is atomic but adding operations isnt
   * ALSO we cannot synchronize using synchronized(this) as every thread will
   * lock on its own object and thus every thread will get the lock providing
   * no benifits there the lock below is used.
   */
  ...
  class ParsingThread implements Runnable {
    ...
    public void run() {
      try {
        ...
        /*
         * Synchronization is necessary to make this operation thread safe other
         * wise we will get an infinite loop.
         * NOTE: Do not synchronize on 'this' as this wont solve the problem as every
         * thread will ask for the lock on its own different object and thus will succeed! 
         */
        synchronized(lock){
          testProgress += mId;
        }
